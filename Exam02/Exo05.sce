...
plot(nouvelles_abscisses, lagrange, 'y')

legend("Points mesurés", "Interpolation Newton", "Interpolation affine", "Interpolation spline", "Interpolation Lagrange")

[valeur, index] = max(polynome)
disp("Max Newton : U* = ", valeur, "t* = ", index)
[valeur, index] = max(affine)
disp("Max affine : U* = ", valeur, "t* = ", index)
[valeur, index] = max(spline)
disp("Max spline : U* = ", valeur, "t* = ", index)
